...
void dir_createDefaultEntry(FileSystem *fs,FileRecord *filerec,eint8* fatfilename)
{
	memCpy(fatfilename,filerec->FileName,LIST_MAXLENFILENAME);
...
euint32 dir_findFileinBuf(euint8 *buf, eint8 *fatname, FileLocation *loc)
{
	...
	for(c=0; c<16; c++)
	{
		...
		{
			if( strMatch((eint8*)fileEntry.FileName,fatname,LIST_MAXLENFILENAME) == 0 )
			{
...
esint8 dir_getFatFileName(eint8* filename, eint8* fatfilename)
{
	eint8 ffnamec[LIST_MAXLENFILENAME],*next,nn=0;
	
	memClr(ffnamec,LIST_MAXLENFILENAME); memClr(fatfilename,LIST_MAXLENFILENAME);
	next = filename;
	
	if(*filename=='/')next++;
	
	while((next=file_normalToFatName(next,ffnamec))){
		memCpy(ffnamec,fatfilename,LIST_MAXLENFILENAME);	
		nn++;
	}
	if(nn)return(1);
	return(0);
